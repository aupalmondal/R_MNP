...
gLA = function(L,A){
  h = nrow(L)
  g = kronecker(gss.eye(h),A)
  r = vectranspose(h,h)
  g = gss.submat(g,r,0)
  g = gss.selif(g,gss.vecr(matdup(gss.ones(h*(h+1)/2,1))))
  return(g)
}

gradmixedprobit = function(xsubq, zsubq,Msubq,b,c,Lsubtau,theta,lambda){
  d = c + t(Lsubtau)%*%theta
  Felem = (-sqrt(2)*Msubq%*%(xsubq%*%b+zsubq%*%d))+lambda
  Felemcdfn = pnorm(Felem)
  Fvalue = apply(Felemcdfn,2,prod) 
  
  ggen = as.vector(dnorm(Felem))
  gb = as.vector(((1/Felemcdfn)%*%Fvalue*(-sqrt(2)))*ggen)*(Msubq%*%xsubq)
  gb = colSums(gb)
  
  gd = as.vector(((1/Felemcdfn)%*%Fvalue%*%(-sqrt(2)))*ggen)*(Msubq%*%zsubq)
  gd = colSums(gd)
  
  gLsubtau = gLA(Lsubtau, theta)
  gLsubtaufinal = as.vector(gLsubtau%*%gd)
  return(list(Fvalue,gb,gd,gLsubtaufinal))
  
}



gomegxomegax <- function(x) {
  g = t(x)%x%t(x) 
  g = gasymtosym(g);
  if (.omegacorr==1) {
    tempselmatrix = vecdup(diag(ncol(x)))
    g = gss.delif(g,tempselmatrix)
    if (.xomegaxt==1) {
      tempselmatrix = vecdup(diag(nrow(x)))
      g = t(gss.delif(t(g),tempselmatrix))
    }
  }
  return(g)
...
ggradm <- function(M) {
  g1 = gomegxomegax(M)
  c = ncol(M)
  temp1 = gss.zeros(c+1,1) %|% gss.ones(((c+1)*(c-2)/2),1)
  g1 = gss.selif(g1,temp1)
  return(g1)
}
